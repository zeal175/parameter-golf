...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.tie_embeddings:
            # Reuse the same fake-quant STE as F.embedding above (tied weight).
...
def main() -> None:
    ...
    qat_frac = min(max(args.qat_final_frac, 0.0), 1.0)
    qat_start = int(args.iterations * (1.0 - qat_frac)) if args.qat_enabled else args.iterations + 1
    log0(
        ...
        f"ema_decay:{args.ema_decay} qat_enabled:{args.qat_enabled} qat_start_step:{qat_start} "
        f"qat_final_frac:{qat_frac}"
    )
